    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
        ...
        <form class="form-horizontal" action="{{ route('articles.update' , ['id' => $article->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-12">
                    <div class="row">
                        @foreach($article->images['image'] as $key => $img )
                            <div class="col-sm-2">
                                <label class="control-label">
                                       {{$key}}
                                       <input type="radio" name="imageThumb" value="{{$img}}" {{$article->images['thumb'] == $img ? 'Checked' : ''}}/>
                                       <a href="{{$img}}" target="_blank"><img src="{{$img}}" width="100%" /> </a>
                                </label>
                            </div>
                         @endforeach
                    </div>
